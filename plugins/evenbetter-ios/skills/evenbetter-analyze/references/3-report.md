...
5. Serialize the data object to JSON. Use the same JSON shape that the template's `reportApp()` function expects: a single object containing `project_name`, `project_path`, `framework`, `wcag_level`, `scan_date`, `summary`, `issues`, and `scan_context`.
