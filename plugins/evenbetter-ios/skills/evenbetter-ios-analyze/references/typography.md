...
Analyze only the `typography` domain. Never edit, write, delete, format, generate, or execute source/project files inside `projectPath`; read files only. Report storage is handled by the top-level analyzer.
