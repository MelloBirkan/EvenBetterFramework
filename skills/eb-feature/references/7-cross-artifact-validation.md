...
All artifacts live in `.evenbetter/<feature-folder>/`. Scan `.evenbetter/` to find the feature folder. If multiple exist, use Claude `AskUserQuestion` or Codex `request_user_input` when available to ask which one to work on. Read and update the plan and tickets in `.evenbetter/<feature-folder>/`.
