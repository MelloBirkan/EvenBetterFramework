...
- Use `Glob`, `Grep`, and `Read` tools to explore the codebase when assessing impact on technical specs.
...
All artifacts live in `.evenbetter/<epic-folder>/`. Scan `.evenbetter/` to find the epic folder. If multiple exist, use Claude `AskUserQuestion` or Codex `request_user_input` when available to ask which epic to work on. Read and update all specs and tickets in `.evenbetter/<epic-folder>/`.
